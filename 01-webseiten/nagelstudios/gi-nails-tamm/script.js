...
function buildRequestText() {
  const parts = ["Hallo GI Nails Team,", "", "ich möchte gerne einen Termin anfragen:"];
  ...
  if (selectedStaff) parts.push(`Mitarbeiterin: ${selectedStaff}.`);
...
function updateSummary() {
  ...
  const staffText = selectedStaff || "Mitarbeiterin offen";
...
function openWhatsappRequest() {
  ...
  const whatsappUrl = `https://wa.me/491632070766?text=${encodeURIComponent(text)}`;
